...
| 상점 아이템 선택 적절하지 않은 입력  | 1. 구매 및 판매시 숫자 외 입력, 범위 밖 입력 등 적절하지 않은 입력<br>2. 에러 메시지 출력 & 재입력 요구 | 비정상(Abnormal) | 
...
| 게임 클리어 시 | 1. 10레벨 도달 후 보스몬스터와 전투<br>2. 승리 시 처치한 몬스터의 종류와 수 출력 후 관련 스토리 출력  | 정상(Normal)     |
