...
몬스터마다 고유한 드롭 확률과 골드량을 설정할 수 있도록 dropRate와 dropGold 변수를 설계에 포함하였으며, 이를 통해 몬스터별로 다른 보상과 드롭 확률을 가질 수 있도록 구현되었습니다.

협업 과정
몬스터의 스펙이 캐릭터 레벨에 비례해서 랜덤하게 생성되도록 하기 위해, 캐릭터 레벨을 매개변수로 받는 구조를 요청하였고, 
랜덤 값 생성을 위한 GetRandomNum(int min, int max) 함수를 Monster 클래스에 구현해달라고 협의했습니다.
또한, dropRate와 dropGold 변수를 활용하여 몬스터마다 고유한 드롭 확률과 보상이 적용되도록 요청하였습니다.
